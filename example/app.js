const { createKoiKoiGame, createGameState, getCard } = window.hanafudaJS
...
const handleCardSelect = (event) => {
  ...
  if (containerId === `player${game.getCurrentPlayer() === "player1" ? "1" : "2"}Hand`) {
    ...
    result = game.selectHandCard(cardIndex)
    if (result.type === "NO_MATCHES") {
      ...
      const { canAutoCapture, matchingCards } = result.data
      // Consumer can choose to auto-capture with 1 or 3 matches
      // if (canAutoCapture) {
      //   handlePlay()
      // } else {
      //   updatePlayButton()  // Let user select one of two matches
      // }
      updatePlayButton()
    }
  } else if (containerId === "fieldCards") {
    result = game.selectFieldCard(cardIndex)
    if (result.type === "ERROR") {
      alert(result.data.message)
      return
    } else if (result.type === "SELECTION_UPDATED") {
      const { canAutoCapture, selectedFieldCards } = result.data
      // Consumer can choose to auto-capture with 1 or 3 matches
      // if (canAutoCapture) {
      //   handlePlay()
      // } else {
  ...
  switch (result.type) {
    ...
    case "ROUND_END":
      if (result.data.winner) {
        ...
        alert(result.data.message || "Round ended! (Deck empty)")
      }
      break
